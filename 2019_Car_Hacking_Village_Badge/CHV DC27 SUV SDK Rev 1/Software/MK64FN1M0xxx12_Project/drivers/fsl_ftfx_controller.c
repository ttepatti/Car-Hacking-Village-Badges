...
status_t FTFx_API_Init(ftfx_config_t *config)
{
    ...
    returnCode = ftfx_init_ifr(config);
    ...
    if (config->ifrDesc.feature.has8ByteIdxSupport != 0U)
    {
        if (config->ifrDesc.feature.has4ByteIdxSupport != 0U)
        {
            if (((index == config->ifrDesc.idxInfo.mix8byteIdxStart) ||
                 ((index >= ((uint32_t)config->ifrDesc.idxInfo.mix8byteIdxStart + 1U)) && (index <= config->ifrDesc.idxInfo.mix8byteIdxStart))) &&
                  (lengthInBytes == 8U))
            {
                                 ...
                                 uint32_t lengthInBytes)
{
    ...
    while (lengthInBytes > 0U)
    {
        ...
        while (lengthTobeProgrammedOfCurrentSector > 0U)
        {
            ...
            memcpy((void *)(uint32_t *)config->flexramBlockBase, &srcaddress[currentOffset], programSizeOfCurrentPass);
                              ...
                              ftfx_swap_state_config_t *returnInfo)
{
    ...
    if (address & (FSL_FEATURE_FLASH_PFLASH_SWAP_CONTROL_CMD_ADDRESS_ALIGMENT - 1u))
    {
...
static status_t ftfx_command_sequence(ftfx_config_t *config)
{
    ...
    /* Since the value of ARM function pointer is always odd, but the real start address
     * of function memory should be even, that's why +1 operation exist. */
    callFtfxRunCommand_t callFtfxRunCommand = (callFtfxRunCommand_t)((uint32_t)config->runCmdFuncAddr + 1u);
                                     ...
                                     uint8_t alignmentBaseline)
{
    ...
    if ((0U != (startAddress & (uint8_t)(alignmentBaseline - 1U))) || (0U != (lengthInBytes & (uint8_t)(alignmentBaseline - 1U))))
    {
        return kStatus_FTFx_AlignmentError;
    }
